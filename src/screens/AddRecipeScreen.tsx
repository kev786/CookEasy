...
const AddRecipeScreen: React.FC<Props> = ({ navigation, route }) => {
  ...
  const calculateNutrition = () => {
    ...
    ingredients.forEach((ing) => {
      ...
      if (data) {
        const weight = qty; // Simplification : assume que la quantité est en grammes ou unités
        calories += (data.caloriesPer100g * weight) / 100;
        budget += (data.pricePer100g * weight) / 100;
      }
    });
  ...
  const saveRecipe = async () => {
    ...
    try {
      ...
      Alert.alert('Erreur', error.message || 'Échec de l\'ajout de la recette.');
    }
  };
      ...
      <ScrollView contentContainerStyle={styles.scrollContentContainer}>
        ...
        <View style={styles.content}>
          ...
          <View style={styles.actionButtonContainer}>
            <TouchableOpacity onPress={saveRecipe} style={styles.publishButtonWrapper}>
              <LinearGradient colors={['#f97316', '#ef4444']} style={styles.publishButton}>
